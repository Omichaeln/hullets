...
export const entriesRouter = router({
  ...
  get: guard("entry.read").input(z.object({ entryId: z.string() })).query(async ({ ctx, input }) => {
    ...
    const s = await ctx.app.pipeline.get(e.submissionId); const participant = await ctx.app.participants.get(e.participantId); const [canonical] = await ctx.app.db.select().from(canonicalReceipts).where(eq(canonicalReceipts.id, e.canonicalReceiptId));
