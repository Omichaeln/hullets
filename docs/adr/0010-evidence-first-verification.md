...
- **The FDMS contract is not verified.** The QR layout and the validation endpoint shape were implemented without access to ZIMRA's published specification or a test device. Both are configuration (`FISCAL_QR_LAYOUT`, `FISCAL_BASE_URL`, `FISCAL_CONTRACT_MODE`) and the adapter is one class. **They must be confirmed against the real specification before this path is trusted in production** — a wrong QR width mis-identifies every receipt, and the parser deliberately refuses a tail whose numeric groups are not numeric rather than producing a silently misaligned identity.

  Work done independently on `main` establishes one fact this ADR originally assumed away: the live ZIMRA endpoint is an HTML **verification page**, not a JSON API, and the invoice detail only appears after submitting the page's "Review Invoice" form with its hidden fields. The adapter therefore supports both shapes — `FISCAL_CONTRACT_MODE=html` (the default) follows that form and reads the resulting page; `json` maps a structured body, which is what a vendor gateway in front of FDMS would return. Reading a page rather than a document is strictly weaker evidence: a redesign turns confirmed values into absent ones. `readHtmlVerdict()` is written so that weakness fails closed — only an explicit affirmative reads as valid, and an unrecognised page is not. This does not relax the requirement above; it narrows what has to be confirmed to the page's field labels and the form's shape.
...
2. Confirm the QR layout and the validation endpoint against ZIMRA's specification and a real fiscal receipt. Adjust `FISCAL_QR_LAYOUT`, `FISCAL_CONTRACT_MODE` and the adapter's request shape. For the HTML contract specifically, capture a real verification page and check the field labels `readHtmlFields()` keys on and the affirmative phrase `readHtmlVerdict()` requires.
