...
export function evaluate(facts: Facts, rules: Rules, ctx: Context): Evaluation {
  ...
  if (!rules.outletMatch.required) add("outlet_match", "pass", null, { skipped: true }); else if (hit) add("outlet_match", "pass", null, hit); else if (!cands.length) add("outlet_match", "unknown", "outlet_unreadable", { header: facts.merchant.text }); else add("outlet_match", "unknown", "outlet_mismatch", { header: facts.merchant.text, top: cands[0] });
