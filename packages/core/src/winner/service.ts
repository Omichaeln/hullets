...
export class WinnerService {
  ...
  private async transitionIn(tx: DbOrTx, w: Winner, to: string, o: { actorId: string; note?: string | null; reason?: string | null; collectionOutletId?: string | null; fulfilmentRef?: string | null; evidence?: string | null }) {
    ...
    await this.deps.crm?.emit(tx, { entityType: "claim", entityId: w.id, entityVersion: w.version + 1, payload: { winnerId: w.id, state: to, collectionOutlet: (o.collectionOutletId ?? w.collectionOutletId) ? (await this.deps.campaigns.outlet((o.collectionOutletId ?? w.collectionOutletId)!))?.code ?? null : null, fulfilledAt: to === "collected" ? ts : null } });
    await this.deps.crm?.emit(tx, { entityType: "winner", entityId: w.id, entityVersion: w.version + 1, payload: { participantId: w.participantId, campaignCode: w.campaignId, period: d?.periodCode, prize: w.prizeLabel, rank: w.position, status: to } });
