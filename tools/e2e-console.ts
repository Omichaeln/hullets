...
try {
  ...
  const PAGES: Array<[string, string]> = [["dashboard", "/"], ["campaigns-new", "/campaigns/new"], ["submissions", "/submissions"], ["submissions-queue", "/submissions?tab=queue"], ["entries", "/entries"], ["participants", "/participants"], ["support", "/support"], ["draws", "/draws"], ["winners", "/winners"], ["campaigns", "/campaigns"], ["master-data", "/master-data"], ["simulator", "/simulator"], ["ops", "/ops"], ["audit", "/audit"], ["staff", "/staff"], ["settings", "/settings"], ["readiness", "/readiness"], ["account", "/account"]];
  for (const role of Object.keys(STAFF) as Array<keyof typeof STAFF>) {
    ...
    for (const [name, url] of PAGES) {
      const allowed = nav.some((t) => url === "/" ? t === "Dashboard" : url === "/campaigns/new" ? t === "Campaigns" : url.startsWith("/submissions") ? t.startsWith("Submissions") : url === "/master-data" ? t.startsWith("Outlets") : url === "/ops" ? t.startsWith("Integrations") : url === "/staff" ? t === "Access" : url === "/settings" ? t === "Settings" : url === "/account" ? t === "My account" : t.toLowerCase().startsWith(url.slice(1).split("?")[0].replace("-", " ")));
  ...
  { const page = await newPage("manager-mobile", true); await step("mobile: sign in and open the menu", async () => { await login(page, "manager"); await page.goto(`${BASE}/submissions`); await settle(page); await shot(page, "manager-submissions-mobile"); await page.getByRole("button", { name: "Open menu" }).click(); await page.waitForTimeout(300); await shot(page, "manager-menu-mobile"); await page.goto(`${BASE}/`); await settle(page); await shot(page, "manager-dashboard-mobile"); }); await page.context().close(); }
