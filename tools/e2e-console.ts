...
try {
  ...
  for (const role of Object.keys(STAFF) as Array<keyof typeof STAFF>) {
    ...
    for (const [name, url] of PAGES) {
      const allowed = nav.some((t) => url === "/" ? t === "Overview" : url.startsWith("/submissions") ? t.startsWith("Submissions") : url === "/master-data" ? t.startsWith("Outlets") : url === "/ops" ? t.startsWith("Integrations") : url === "/staff" ? t === "Access" : url === "/account" ? t === "My account" : t.toLowerCase().startsWith(url.slice(1).split("?")[0].replace("-", " ")));
